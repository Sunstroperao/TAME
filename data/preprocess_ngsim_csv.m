...
us101_1 = '/MATLAB Drive/ngsim_dataset/trajectories-0750am-0805am.csv';
us101_2 = '/MATLAB Drive/ngsim_dataset/trajectories-0805am-0820am.csv';
us101_3 = '/MATLAB Drive/ngsim_dataset/trajectories-0820am-0835am.csv';
i80_1 = '/MATLAB Drive/ngsim_dataset/trajectories-0400-0415.csv';
i80_2 = '/MATLAB Drive/ngsim_dataset/trajectories-0500-0515.csv';
i80_3 = '/MATLAB Drive/ngsim_dataset/trajectories-0515-0530.csv';
...
disp('Loading data...')
traj{1} = readtable(us101_1);
traj{1} = traj{1}{:,1:14};
traj{1} = single([1*ones(size(traj{1},1),1),traj{1}]);
traj{2} = readtable(us101_2);
traj{2} = traj{2}{:,1:14};
traj{2} = single([2*ones(size(traj{2},1),1),traj{2}]);
traj{3} = readtable(us101_3);
traj{3} = traj{3}{:,1:14};
traj{3} = single([3*ones(size(traj{3},1),1),traj{3}]);
traj{4} = readtable(i80_1); 
traj{4} = traj{4}{:,1:14};
traj{4} = single([4*ones(size(traj{4},1),1),traj{4}]);
traj{5} = readtable(i80_2);
traj{5} = traj{5}{:,1:14};
traj{5} = single([5*ones(size(traj{5},1),1),traj{5}]);
traj{6} = readtable(i80_3);
traj{6} = traj{6}{:,1:14};
...
%save('allData.mat','traj');


%% Split train, validation, test
%load('allData.mat','traj');
