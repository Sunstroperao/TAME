...
class NgsimDataset(Dataset):
    ...
    def collate_fn(self, samples):
        ...
        pos = [0, 0]
        mask_batch = torch.zeros(len(
            samples), self.grid_size[1], self.grid_size[0], self.enc_size)  # (batch,3,13,h)
        temporal_mask_batch = torch.zeros(len(
            samples), self.grid_size[1], self.grid_size[0], 6)  # (batch,3,13,h)
        ...
        fut_batch = torch.zeros(self.t_f // self.d_s,
                                len(samples), 2)  # (len2,batch,2)
        op_mask_batch = torch.zeros(
            self.t_f // self.d_s, len(samples), 2)  # (len2,batch,2)
        ...
        for sampleId, (hist, fut, nbrs, lat_enc, lon_enc, va, neighborsva, lane, neighborslane, refdistance,
            ...
            for id, nbr in enumerate(nbrs):
                if len(nbr) != 0:
                    ...
                    temporal_mask_batch[sampleId, pos[1],
                                        pos[0], :] = torch.ones(6).byte()
        ...
        return hist_batch, nbrs_batch, mask_batch, lat_enc_batch, lon_enc_batch, fut_batch, op_mask_batch, va_batch, nbrsva_batch, lane_batch, nbrslane_batch, distance_batch, \
            nbrsdis_batch, class_batch, nbrsclass_batch, map_position, nbrs_num_batch, temporal_mask_batch
...
if __name__ == '__main__':
    ...
    trLD = torch.utils.data.DataLoader(trSet, batch_size=128, shuffle=True, collate_fn=trSet.collate_fn)
    for i, data in enumerate(trLD):
        hist, nbrs, mask, lat_enc, lon_enc, fut, op_mask, va, nbrsva, lane, nbrslane, _, _, cls, nbrscls, ref_position, nbrs_num, temporal_mask = data
        print(hist.shape, nbrs.shape, mask.shape, lat_enc.shape, lon_enc.shape, fut.shape, op_mask.shape, va.shape, nbrsva.shape, lane.shape, nbrslane.shape, cls.shape, nbrscls.shape, ref_position.shape, nbrs_num.shape, temporal_mask.shape)
        break