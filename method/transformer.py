...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer, num_layers, norm=None):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)  # 克隆encoder_layer，克隆num_layers次
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False):
        super().__init__()
        self.layers = _get_clones(decoder_layer, num_layers)  # 克隆decoder_layer，克隆num_layers次
        self.num_layers = num_layers
        self.norm = norm
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.self_attn(q, k, value=tgt, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]  # self_attn返回值是一个元组，第一个元素是输出，第二个元素是attention矩阵
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
            ...
            0]  # multihead_attn返回值是一个元组，第一个元素是输出，第二个元素是attention矩阵
        tgt = tgt + self.dropout2(tgt2)
        tgt = self.norm2(tgt)  # LayerNorm
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))  # Feedforward
        tgt = tgt + self.dropout3(tgt2)
        tgt = self.norm3(tgt)  # LayerNorm
        return tgt
    ...
    def forward_pre(self, tgt, memory,
        ...
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt2))))  # Feedforward
        tgt = tgt + self.dropout3(tgt2)
        return tgt
